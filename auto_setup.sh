...
# =========================================================================
# 🤖 CloudBot + Shizuku Universal Installer (Fixed)
# =========================================================================
...
echo ""
echo "🤖 CloudBot Non-Root Phone Control Installer (Fixed)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

# =========================================================================
# Step 1/6: Update Packages & Install Dependencies
# =========================================================================
echo "📦 Step 1/6: Updating packages and installing dependencies..."
...
# =========================================================================
# Step 2/6: Setup Shizuku (rish & shizuku commands)
# =========================================================================
echo ""
echo "🔒 Step 2/6: Linking Shizuku to Termux..."
...
# =========================================================================
# Step 3/6: Fix Node.js IPv4 DNS (Crucial for Termux)
# =========================================================================
echo ""
echo "🔧 Step 3/6: Applying Network Fixes..."
...
# =========================================================================
# Step 4/6: Install Official OpenClaw
# =========================================================================
echo ""

if command -v openclaw &>/dev/null || [ -d "$HOME/.openclaw/repo" ]; then
    echo "✅ Step 4/6: OpenClaw is already installed! Skipping installation."
else
    echo "📦 Step 4/6: Installing OpenClaw. This takes a few minutes..."
    bash -c "$(curl -sSL https://myopenclawhub.com/install)" </dev/null && source ~/.bashrc 2>/dev/null
fi

# =========================================================================
# Step 5/6: Fix Missing OpenClaw npm Dependencies
# =========================================================================
echo ""
echo "🔧 Step 5/6: Fixing missing OpenClaw npm dependencies..."

# Locate the global openclaw package directory
OPENCLAW_PKG_DIR=""
for candidate in \
    "/data/data/com.termux/files/usr/lib/node_modules/openclaw" \
    "$(npm root -g 2>/dev/null)/openclaw" \
    "$HOME/.openclaw/repo"; do
    if [ -d "$candidate" ]; then
        OPENCLAW_PKG_DIR="$candidate"
        break
    fi
done

if [ -n "$OPENCLAW_PKG_DIR" ]; then
    echo "   Found OpenClaw at: $OPENCLAW_PKG_DIR"
    cd "$OPENCLAW_PKG_DIR"

    # Resolve the global npm root once to avoid repeated process spawning
    NPM_GLOBAL_ROOT=$(npm root -g 2>/dev/null)

    # Install known missing peer dependencies that cause startup failures
    DEPS_TO_FIX="@buape/carbon @slack/web-api"
    MISSING_DEPS=""
    for dep in $DEPS_TO_FIX; do
        if [ ! -d "node_modules/$dep" ] && [ ! -d "$NPM_GLOBAL_ROOT/$dep" ]; then
    ...
    if [ -n "$MISSING_DEPS" ]; then
        ...
        npm install $MISSING_DEPS </dev/null 2>&1 || {
            echo "⚠️  npm install had warnings — trying individual installs..."
            for dep in $MISSING_DEPS; do
                npm install "$dep" </dev/null 2>&1 || \
                    echo "⚠️  Could not install $dep — you may need to run: cd $OPENCLAW_PKG_DIR && npm install $dep"
            done
        }
    else
        echo "   All required dependencies are already present."
    fi
    cd - > /dev/null
else
    echo "⚠️  OpenClaw package directory not found. Skipping dependency fix."
    echo "   If openclaw errors with 'Cannot find module', run:"
    echo "   cd \$(npm root -g)/openclaw && npm install @buape/carbon @slack/web-api"
fi

echo "✅ Dependency fix complete"

# =========================================================================
# Step 6/6: Inject Shizuku Phone Control Scripts & AI Override
# =========================================================================
echo ""
echo "🧠 Step 6/6: Configuring AI Phone Controller..."
...
echo ""
echo "🐛 If you still see 'Cannot find module' errors after setup:"
echo "   Run: cd \$(npm root -g)/openclaw && npm install @buape/carbon @slack/web-api"
echo ""
